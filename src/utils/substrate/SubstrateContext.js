...
const connect = (state, dispatch) => {
  ...
  _api.on("connected", () => {
    ...
    console.log("=========== connected ==========", _api);
    dispatch({ type: "CONNECT", payload: _api });

    _api.isReady.then((_api) => {
      console.log("=========== CONNECT_SUCCESS ==========", _api);
  ...
  _api.on("error", (err) => {
    console.log("=========== CONNECT_ERROR ==========", _api);
...
const SubstrateContextProvider = (props) => {
  ...
  useEffect(() => {
    ...
    if (apiState === "READY" && !keyringState && !keyringLoadAll) {
      keyringLoadAll = true;
      state?.currentAccount?.address && loadAccounts(state, dispatch, state?.selectedExtension);
